...
class AttentionNPContextualEmbedder(BaseNPContextualEmbedder):
    def __init__(self, attention: bool, d_model, nhead, num_layers, dim_feedforward=2048, dropout=0.1):
        super().__init__()
        self.attention = attention
        if attention:
            encoder_layer = torch.nn.TransformerDecoderLayer(d_model, nhead, dim_feedforward, dropout)
            self.transformer = torch.nn.TransformerDecoder(encoder_layer, num_layers)
        else:
